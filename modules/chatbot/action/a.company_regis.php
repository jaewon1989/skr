...
if (!$_SESSION['wcode']){
	...
	if ($uid)
	{
		$R = getUidData($table[$m.'company'],$uid);
		if (!$R['uid']){
		...
		$QVAL = "subject='$subject',content='$content',links='$links',area_depth='$area_depth',category='$category',sido='$sido',gugun='$gugun',dong='$dong',d_start='$d_start',d_end='$d_end',d_modify='$d_regis',tel='$tel',tel2='$tel2',email='$email'";
		
		getDbUpdate($table[$m.'company'],$QVAL,'uid='.$R['uid']);
	...
	{	    
		$mingid = getDbCnt($table[$m.'company'],'min(gid)','');
		$gid = $mingid ? $mingid-1 : 1000000000;
     
 		$QKEY = "auth,gid,ad_type,display,user_display,hidden,notice,name,nic,register,mbruid,id,area_depth,category,sido,gugun,dong,subject,content,html,";
		$QKEY.= "hit,down,comment,likes,unlikes,report,point,d_start,d_end,d_regis,d_modify,upload,ip,agent,sync,sns,adddata,links,is_onair,tel,tel2,email";

		$QVAL = "'$auth','$gid','$ad_type','$display','$user_display','$hidden','$notice','$name','$nic','$register','$mbruid','$id','$area_depth','$category','$sido','$gugun','$dong','$subject','$content','$html',";
		$QVAL.= "'$hit','$down','$comment','$likes','$unlikes','$report','$point','$d_start','$d_end','$d_regis','$d_modify','$upload','$ip','$agent','$sync','$sns','$adddata','$links','$is_onair','$tel','$tel2','$email'";

		getDbInsert($table[$m.'company'],$QKEY,$QVAL);
		
		$LASTUID = getDbCnt($table[$m.'company'],'max(uid)','');
	...
	// 태그 등록 함수 실행 
	if ($tag || $R['tag']) $Ad->RegisPostTag($tag,$R,'company',$moduleid,$reply,$NOWUID);
} 

// 어드민 등록시 
if($regis_mod=='admin'){
	$adm_link = $g['s'].'/?m=admin&module='.$m.'&front=company';
