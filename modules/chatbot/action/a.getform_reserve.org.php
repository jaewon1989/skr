...
if(!trim($_POST['pData']['bot']) || !trim($_POST['pData']['r_data'])) {
...
function getReserveScheduleDate($data) {
    ...
    if(is_array($apiResult['times']) && count($apiResult['times']) > 0) {
        ...
        $TMPL['data_row'] = "<div class='acenter mt10 mb10'>희망하신 날짜의 예약이 불가능합니다.<br>다른 날짜를 선택해주세요.</div>";
    }    
...
function getReserveDoctor($data) {
    ...
    if($apiResult['result']) {
        ...
        foreach($apiData as $aData) {
            ...
            if($aData['image']) {                
                if(parse_url($input, PHP_URL_SCHEME) != "https") {
            ...
            $image = $aData['image'] ? "style='background-image:url(".$aData['image'].")';" : "";
...
function getReserveInfoHtml($data) {
    ...
    $aItem['hotel'] = array(
        "customer_name"=>"예약자명", "reservation_status"=>"예약상태", "created_at"=>"예약일시", "checkin"=>"체크인", "checkout"=>"체크아웃", "room_name"=>"예약객실명", 
        "room_price"=>"예약인원", "goods_price"=>"상품", "total_price"=>"총결제금액", "payment_status"=>"결제상태", "payment_method"=>"결제방법", "description"=>"요청사항"
    );
    $html = "";
...
$r_data = json_decode(stripslashes($pData['r_data']), true);
                ...
                if(!isset($apiResult['result']) && !$apiResult['result']) {
                    ...
                    foreach($aRoomTypes as $aRoom) {
                        ...
                        foreach($aRoom['rateplans'] as $aRate) {
                            ...
                            $html .="            <span class='rate_price'>".number_format($aRate['total_price'])."원</span>";
                            ...
                            $html .="                <span>총</span><span><em>".number_format($aRate['total_price'])."</em>원</span>";
        ...
        case('hotel_room') :
            ...
            if(count($apiResult) > 0) {            
                ...
                foreach($apiResult as $aGoods) {                    
                    ...
                    $html .="        <span class='fr aright des'>".number_format($aGoods['basic_price'])."원</span>";
                    $html .="    </button>";
                    ...
                    $html .="        </div>";
                    $html .="        <div class='fr'>";
                    $html .="            <span class='goods_price'><em>".number_format($aGoods['basic_price'])."</em>원</span>";
                ...
                $html .="<li><div><span class='item'>객실료</span><div class='cont'>".number_format($r_data['hotel_room_basic_price'])."원</div></div></li>";
                if($r_data['hotel_room_add_adult_price']) {
                    $html .="<li><div><span class='item'>성인추가요금</span><div class='cont'>".number_format($r_data['hotel_room_add_adult_price'])."원</div></div></li>";
                }
                if($r_data['hotel_room_add_child_price']) {
                    $html .="<li><div><span class='item'>아동추가요금</span><div class='cont'>".number_format($r_data['hotel_room_add_child_price'])."원</div></div></li>";
                }
                $html .="<li><div><span class='item'>총금액</span><div class='cont'>".number_format($r_data['hotel_room_price'])."원</div></div></li>";
        ...
        case('hotel_goods') :
            ...
            if(count($r_data['hotel_room_goods']) > 0) {
                foreach($r_data['hotel_room_goods'] as $idx=>$aGoods) {
                    ...
                    $html .="<li><div><span class='item'>".$goods_name."</span><div class='cont'>".number_format($aGoods['price'])."원</div></div></li>";
                }
            }
            $html .="<li><div><span class='item'>총금액</span><div class='cont'>".number_format($r_data['hotel_room_all_price'])."원</div></div></li>";
            $TMPL['reserve_cart_data'] = $html;
    ...
    if($step == "auth") {
        if($category == "hotel") {
            ...
            if(!isset($apiResult['result']) && !$apiResult['result']) {        
                ...
                $_r_data['room_name'] = $reservation_items['room_name'];
                ...
                $_r_data['room_price'] .=" (".number_format($reservation_items['total_amount'])."원)";
                ...
                if(isset($reservation_items['reservation_item_products']) && $reservation_items['reservation_item_products'] > 0) {                    
                    foreach($reservation_items['reservation_item_products'] as $aGoods) {
                        $_r_data['goods_price'] .=$aGoods['product']['name']." ".$aGoods['quantity']." (".number_format($aGoods['total_amount'])."원)<br>";
                    }
                    $_r_data['goods_price'] = rtrim($_r_data['goods_price'], "<br>");
                }
                $_r_data['total_price'] = number_format($apiResult['total_amount'])."원";
                ...
                if($apiResult['payment_method'] == "wired") {
                    $_r_data['payment_method'] .="<br>(".$apiResult['property']['be_bank_name']." ".$apiResult['property']['be_bank_account']." ".$apiResult['property']['be_depositor'].")";
                }
...
if($last_chat && $result['log'] && $result['msg']) {
    $botChat = array();
