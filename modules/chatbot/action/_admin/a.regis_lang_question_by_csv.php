...
getDbDelete($chatbot->tbl_question,'vendor=1');
getDbDelete($chatbot->tbl_rule,'vendor=1'); 
...
foreach($lines as $line)
{
    if($i>0){
    	$row = explode(',',$line);   
	    $q_uid = trim($row[0]); // 질문 uid
	    $question = trim($row[1]);// 질문  
	    $Q = getDbData($table[$m.'question'],'uid='.$q_uid,'*');
	    $quesCat = $Q['quesCat']; 
	    $q_data = array(
		        "vendor"=>$vendor,
		        "bot" =>$bot,
		        "quesCat"=>$quesCat,
		        "question"=>str_replace('(***)',',',addslashes($question)),// csv 파일 작업시 문장속 콤마 escape 했던것 복원 
	     );
