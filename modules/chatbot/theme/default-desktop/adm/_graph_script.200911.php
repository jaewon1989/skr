...
<script type="text/javascript">
    ...
    var dialogNodeJson = <?=($dialogNodeJson ? "'".$dialogNodeJson."'" : '{"dialogNodeUid":"0","nodeid":"1","name":"인사","parent":"0"}')?>; 
...
<script type="text/javascript">
    	...
    	if (!mxClient.isBrowserSupported()) {
    		...
    		graph.addMouseListener({
    			...
    			hoverStyle: function(state, hover) {
    				var cell = state.cell;
    				if(cell.is_unknown!='1'){ // 마지막 대화상자가 아닌 경우에만 
    ...
    mxVertexToolHandler.prototype.redrawTools = function() {
    	if (this.state != null && this.domNode != null) {
    		...
    		/*
    		// 스크롤 상태 아닐 때 적용
    		var sTop = (this.state.y + this.state.height + dy);
    		if((sTop + $(this.domNode).height()) > $(this.graph.container).height()) {
    		    sTop = ((sTop - $(this.domNode).height()) + this.state.height);
    		    this.domNode.style.top = sTop + 'px';
    		} else {
    		    this.domNode.style.top = sTop + 'px';
    		} 
    		*/   		
    ...
    function jsonToVertex(dialogNode, source) {
    	var target;
    	var nodeName = dialogNode.name.length > 6 ? dialogNode.name.substr(0, 6)+'...' : dialogNode.name;
