...
$callEntity = '사전명'; // html/inputFilter_inputBox  placeholder  부분 별도 수정 
?>
...
function main(container)
{
  ...
  //       mxEvent.addListener(saveGraph, 'click', function()
        ...
        // panel_action.js 초기화
        var data ={nodeObj:null,act:null,mxAct:false}; 
        initSettingPanel(data); 
	...
	<div id="header">
		...
		<ul id="headerBtnBox">
			...
			<li data-role="change-panelMod" data-type="intent">
				<button type="button" class="btn btn-default" data-tooltip="tooltip" title="문장에 포함된 사용자 의도" ><?php echo $callIntent?></button>
			</li>
			<li data-role="change-panelMod" data-type="entity">
				<button type="button" class="btn btn-default" data-tooltip="tooltip" title="의도와 관련된 핵심 엔터티" ><?php echo $callEntity?></button>
			</li>
			<li data-role="change-panelMod" data-type="chat">
				<span class="header-btn icon-comment" data-tooltip="tooltip" title="테스트" />
					<img src="<?php echo $g['img_layout']?>/icon_comment.png" />
				</span>
			</li>
		</ul>	
	</div>
...
var init_func =function(){
	...
	$('#guide-input').tooltip({
		placement: "right",
		html: true,
        title: '<div class="guid-input"><h6>사용자 입력문장에서 체크된 값</h6><ul class="guide-ul"><li><strong>'+callIntent+'</strong> : 문장에 포함된 의도</li><li><strong>'+callEntity+'</strong> : '+callIntent+'와 관련된 핵심 단어</li></ul></div>',
    });
}
