...
(function (factory) {
    ...
    var KRE_Panel = {
        ...
        controlIntentPanel: function(data){
            ...
            // 인텐트 클릭 이벤트     
            $(Left).on('click','[data-role="intent-panelItem"]',function(){
        ...
        controlEntityPanel: function(data){
            ...
            // 엔터티 클릭 이벤트     
            $(Left).on('click','[data-role="entity-panelItem"]',function(){
                var uid = $(this).data('uid');
                var name = $(this).data('name');
                var type = $(this).data('type');
                $(nameEle).val(name);
        ...
        saveNode: function(){
            ...
            var recognize = $('[name="recognize[]"]').map(function(){return $(this).val()}).get(); // 조건 
            var resGroupHeader = $('[name="respondGroupHeader[]"]').map(function(){return $(this).val()}).get(); // 응답그룹 헤더 
            var resGroupBody = {};
            ...
            var getItemResGroup = function(item){
                ...
                $.each(header,function(i,type){
                    var input = $(item).find('[data-role="res-'+type+'"]');
            ...
            data = {
                 ...
                 "itemResHeaderActive" : itemResHeaderActive
            }; 
        ...
        getMultiMenuResGroup: function(data){
            ...
            var guideText;
            var HeaderData = {
                "text": ["텍스트",'<i class="fa fa-file-text-o"></i>'],
                "link": ["링크",'<i class="fa fa-link"></i>'],
                "img": ["이미지",'<i class="fa fa-picture-o"></i>'],
                "tel": ["전화",'<i class="fa fa-phone"></i>'],
                "node": ["대화상자",'<i class="fa fa-sitemap"></i>']
            };
            // 헤더 배열 배치 
            ...
            if(resType=='hMenu'){
                guideText = '본 메뉴 선택시 액션';
            }else if(resType=='if'){
                guideText = '본 조건 해당시 액션';
            }
        ...
        addInputFilter: function(e){
            ...
            setTimeout(function(){
                var newEle = $(self.options.filterInputBox).last();
        ...
        setFilterData: function(e){
            ...
            $(activeRecognize).val(filterMark+'|'+filterUid+'|'+filterName);
            setTimeout(function(){
                self.hideFilterBox();
            },10);
        ...
        setInputFilterAddBtn: function(){
            var self = this;
            var lastInputFilter = $(this.options.filterInputBox).last();
        ...
        if_setFilterData: function(e){
            ...
            // 선택한 값 저장  
            $(activeInput).val(filterLabel);
            $(activeRecCondition).val(filterMark+'|'+filterUid+'|'+filterName);
        ...
        if_setInputFilterAddBtn: function(data){
            ...
            var inputFilter = $(lastInputFilter).find('input[name="if_recognize[]"]');
            // 마지막 inputBox input 에 값이 있는 경우에 add btn 추가 
            if($(inputFilter).val()){                
        ...
        if_addInputFilter: function(e){
            ...
            var new_order = parseInt(order)+1;
            var inputFilterWrapper = $(target).parent().parent();
            ...
            // 본인 자신 삭제 (플러스 아이콘)             
            setTimeout(function(){                
                $(inputFilterWrapper).find('[data-role="add-ifInputFilter"]').remove();                  
            },50);
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    if(linkType=='getIntent') self.intent = result.content;
                        ...
                        self.linkServerData(data);
                        setTimeout(function(){
                            self.showNotify('#action-form','저장되었습니다.');
                        },100);
                        setTimeout(function(){
                            self.initFunc();
                        },300);
                        ...
                        $(resultContainer).html(result.content);
                        $(dataTableContainer).css({"width": result.width});
