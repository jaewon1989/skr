...
class Chatbot extends Module_base{
    ...
    public $csChatAPIs = array(
        'nexus' => array('name'=>'넥서스', 'url'=>"https://biztalk.nexuscommunity.kr/persona"),
    );
    ...
    function getResponseHint($data){
        ...
        // FAQ 등록된 경우 FAQ 데이터 출력
        $faq_rows = getDbRows($table[$this->module.'faq'],'vendor='.$data['vendor'].' and bot='.$data['bot']);
        if($faq_rows > 0) {
            $data['all_data'] = true;
            $faqResult = $this->getFaqResponse($data);

            $hintList = '';
            foreach($faqResult as $i=>$_data) {
                if($i == 0) $hintList .='<ul class="list-group">';
                $hintList .='<li class="list-group-item"><span style="font-weight:bold;">'.$_data['question'].'</span><br>'.nl2br($_data['answer']).'</li>';
            }
            if($hintList) {
                $hintList .='</ul>';
                $result = $hintList;
            } else {
                $result = '<p>응답 힌트를 찾지 못했습니다.</p>';
            }
        } else {
            ...
            if($E['has_entity']){
                ...
                $UL_start ='<ul class="list-group">';
                $hintList = '';
                foreach ($entityArray as $entityData) {
                    $entityMatch = $entityData[2]; // 매칭단어
                    $entityVal = $entityData[3]; // 밸류
                    $entityName = $entityData[5]; // 엔터티명

                    $_wh = "vendor='".$vendor."' and bot='".$bot."' and resType='text' and (content like '%".$entityMatch."%' or content like '%".$entityVal."%' or content like '%".$entityName."%')";
                    $RCD = getDbSelect($table[$m.'dialogResItem'],$_wh,'content');
                    while($R = db_fetch_array($RCD)){
                        $hintList.= '<li class="list-group-item">'.$R['content'].'</li>';
                    }

                    $_wh = "vendor='".$vendor."' and bot='".$bot."' and resType='text' and (text_val like '%".$entityMatch."%' or text_val like '%".$entityVal."%' or text_val like '%".$entityName."%')";
            ...
            if($hintList!='') $result = $UL_start.$hintList.$UL_end;
    ...
    */
    ...
    function getBotCategory($data){
        ...
        $cat = array();
        $RCD = getDbArray($tbl,'uid=17','uid,name','gid','asc','',1);
        while($R = db_fetch_array($RCD)) {
    ...
    function getApiResponse($data){
        ...
        $dt['stt_language'] = isset($B['stt_language']) ? $B['stt_language'] : 'ko_KR';
        $dt['bot'] = $B['bot'];
        $dt['botId'] = $data['botId'];
        ...
        /*
         * chatbot.class에서 호출 시 필요한 코드
         * $data['node']는 세팅이 안됨으로 무조건 ProcessInput() 호출
         * processInput() => getDialogMatch() 실행 시 lastNodeId가 변경됨으로
         * 현재 lastNodeId를 최신데이터로 보고 데이터 추출
         */
        if (isset($this->lastNodeId)) {
            $nodeSettingInfo = $this->getNodeSettingInfo($dt['bot'], $this->lastNodeId);
            $dt['exceededMsg'] = $nodeSettingInfo['exceededMsg'];
            $dt['exceededCount'] = $data['noMatchExceededCount'];
            $dt['unrecognizedMsg'] = $nodeSettingInfo['unrecognizedMsg'];
            $dt['unrecognizedCnt'] = $nodeSettingInfo['unrecognizedCount'];
            $dt['failMsg'] = $nodeSettingInfo['failMsg'];
        }
        ...
        if(is_array($response)){
            foreach ($response as $i => $resItem){
                ...
                if(is_array($resItem[0])){
                    ...
                    $itemTtsSpeed = $resItem[0]['ttsSpeed'] ?? '0';
                    $itemRData = $resItem[0]['r_data'];
                    $result[] = array("type"=>$itemType,"content"=>$itemCont, "bargein"=>$itemBargein, "next_status"=>$itemNextStatus, "ttsSpeed"=>$itemTtsSpeed, "r_data"=>$itemRData, "unknown"=>$unknown, "res_end"=>$res_end);
                }else{
                    $itemType = $resItem[0];
                    $itemCont = $resItem[1];
    ...
    function updateBotData($data){
        ...
        $callno = str_replace("-", "", $data['callno']);
        $errorMsg = nl2br(str_replace(array("\r\n", "\r", "\n"), "<br>", $data['error_msg']));
        ...
        if($data['page'] =='adm/config'){
            ...
            $upSql = "active='$active',induCat='$induCat',name='$name',service='$service',intro='$intro',website='$website',avatar='$uploadFile',callno='$callno',error_msg='$errorMsg'";
            getDbUpdate($table[$m.'bot'],$upSql,'uid='.$data['uid']);

            // 2024.02.20 spikecow timeout 관련 설정값 추가
            if(isset($data['timeout'])){
                $_data = array();
                $_data['nameArray'] = array();
                $_data['nameArray']['timeout'] = $data['timeout'] == '' ? '0' : $data['timeout'];
                $_data['nameArray']['timeout_msg'] = nl2br(str_replace(array("\r\n", "\r", "\n"), "<br>", $data['timeout_msg']));
                $_data['data'] = $data;
       ...
       // 기본 컨텍스트 값 업데이트
        ...
        // 장애접수
        if(isset($data['use_disability'])){
            $_data = array();
            $_data['nameArray'] = array();
            $_data['nameArray']['use_disability'] = $data['use_disability'];
            $_data['data'] = $data;
            $this->updateBotSettings($_data);
        }
        ...
        if(isset($data['use_chatgpt'])){
            ...
            $_data['nameArray']['msg_chatgpt'] = trim($data['msg_chatgpt']);
            $_data['data'] = $data;
            $this->updateBotSettings($_data);
        }

        // Quick Menu use
        //if(isset($data['menu_quick'])){
            $_data = array();
            $_data['nameArray'] = array();
            //$_data['nameArray']['use_quick'] = trim($data['use_quick']);
            $_data['nameArray']['menu_quick'] = trim($data['menu_quick']);
            $_data['data'] = $data;
            $this->updateBotSettings($_data);
        //}
    ...
    */
    ...
    function getFilterLabelDetail($labelData){
        ...
        if($match[0]){
            ...
            $operator = $match[0]; // :
            $term = $label_arr[1]; // 벨류
            $result = str_replace($operator . $term, '', $labelData).'|'.$operator.'|'.$term;
        }
    ...
    function regisNode($data){
        ...
        $dialog = $data['dialog'];
        $name = $data['nodeName'];
        $parent = $data['nodeParent'] ?? 0;
        $node_action = $data['nodeAction'] ?? 1;
        $jumpTo_node = $data['jumpTo_node'] ?? 0; // node_action 이 2인 경우
        $is_unknown = $data['is_unknown'] ?? 0;// 대화상자 못찾은 경우 응답하는 대화상자 여부값
        $use_topic = $data['use_topic'] ?? 0; // 연결한 토픽(dialog) uid
        ...
        if($is_row){
           ...
           $last_id = $data['node'];
            ...
            if($vendor && $bot && $dialog){
                ...
                $QVAL ="'$gid','$parent','$depth','0','$id','$name','$vendor','$bot','$dialog','$recCondition','$context','$recQry','$track_flag','$node_action','$jumpTo_node','$is_unknown','$use_topic','$d_regis'";
    ...
    function regisNodeConfig($data){
        ...
        $upQry  = "unrecognized_count = '".$data['unrecognizedCount']."',";
        $upQry .= "unrecognized_msg = '".$data['unrecognizedMsg']."',";
        $upQry .= "exceeded_msg = '".$data['exceededMsg']."',";
        $upQry .= "fail_msg = '".$data['failMsg']."',";
        $upQry .= "timeout = '".$data['timeout']."',";
        $upQry .= "timeout_msg = '".$data['timeoutMsg']."'";
        //$upQry .= "tts_speed = '".$data['ttsSpeed']."'";
    ...
    function regisResGroupBody($data){
        ...
        foreach ($data['resGroupBody'] as $val){
            ...
            if($R['uid']){
                ...
                if(isset($data['is_unknown']) && $data['is_unknown']) {
                    $upQry.=",content='".$content."'";
                } else {
                ...
                if(isset($ttsSpeed)) $upQry.=",tts_speed='".$ttsSpeed."'";
                ...
                $QKEY ="gid,vendor,bot,dialog,node,resGroupId,resType,id,title,summary,content,img_url,link1,link2,link3,recQry,recCondition,ctx_init,ctiaction,tts_speed";
                ...
                $QVAL.="'$ctx_init','$ctiaction','$ttsSpeed'";
            ...
            if($item['itemResGroup']){
                ...
                foreach ($item['itemResGroup'] as $type => $uid_val){
                    ...
                    $_data['ttsSpeed'] = '0';
                    if ('text' === $type || 'form' === $type) {
                        $_data['ttsSpeed'] = $uid_val['tts_speed'] ?? '0';
                    }
                    if ('link' === $type) {
                        $_data['linkType'] = $uid_val['link_type'];
                    }
                    $_data['gid'] = $j;
    ...
    function regisItemResGroup($data){
        ...
        if($uid){
            ...
            else if($resType == 'link' && $linkType == 'blank'){
                $val = 'javascript:'.$val;
            }
            ...
            if(!$is_row['uid']){
                $QKEY = "vendor,bot,item,gid,resType,text_val,varchar_val,ctiaction,tts_speed";
                $QVAL = "'$vendor','$bot','$item','$gid','$resType','$text_val','$varchar_val','".$data['ctiaction']."','".$data['ttsSpeed']."'";
                getDbInsert($tbl,$QKEY,$QVAL);
    ...
    // 노드 설정 업데이트 함수
    function saveNodeConfigData($data){
        // 다이얼로그 Node 저장/수정
        return array('node'=> $this->regisNodeConfig($data));
    }
    ...
    function getRecognizeHtml($rec,$mod,$i){
        ...
        $TMPL['input_order'] = $i;
        $TMPL['filter_label'] = $rec_arr[5] . ($rec_arr[6] ?? '') . ($rec_arr[7] ?? '');
    ...
    function getNodeData($data){
        ...
        if($R['uid']){
            ...
            $configGroup = array();
            $configGroup['unrecognizedCount'] = $R['unrecognized_count'];
            $configGroup['unrecognizedMsg'] = $R['unrecognized_msg'];
            $configGroup['exceededMsg'] = $R['exceeded_msg'];
            $configGroup['failMsg'] = $R['fail_msg'];
            $configGroup['timeout'] = $R['timeout'];
            $configGroup['timeoutMsg'] = $R['timeout_msg'];
            $configGroup['dialogName'] = $R['name'];
    ...
    function getResGroupBodyHtml($R,$resGroup){
        ...
        $use_ttsSpeed = $aBargein['ttsSpeed'];
        ...
        foreach ($rows as $index=>$row) {
            ...
            $row['use_ttsSpeed'] = $use_ttsSpeed && ($row['resType'] == 'text' || $row['resType'] == 'if') ? true : false;
            ...
            if($R['tmod']){
                ...
                //TTS Speed
                if($row['use_ttsSpeed']) {
                    $ttsSelectVal = str_replace('-', 'minus', $row['tts_speed']);

                    $TMPL['TTS_item_type'] = 'item';
                    $TMPL['TTS_item_uid'] = $row['uid'];
                    $TMPL['TTS_value'] = $row['tts_speed'];
                    $TMPL['TTS_select_'.$ttsSelectVal] = 'selected';

                    $checkbox_ttsSpeed = new skin('dialog/tts_speed_checkbox');
                    $TMPL['checkbox_ttsSpeed'] = $checkbox_ttsSpeed->make();

                    $TMPL['TTS_select_'.$ttsSelectVal] = '';
                }

            }else{
                $TMPL['ta_rows'] = '10';
                $TMPL['checkbox_dLabel'] = $TMPL['checkbox_bargein'] = $TMPL['checkbox_ctiaction'] = $TMPL['checkbox_ttsSpeed'] = '';
            }
            ...
            if($row['resType']=='hMenu' || $row['resType']=='if' || $row['resType']=='card'){
                // if: 조건 관련 부분
                if($row['resType']=='if'){
                    if($row['recCondition']){
    ...
    function getMenuItemResGroup($row){
        ...
        // 가이드 문구
        if($row['resType']=='hMenu' || $row['resType']=='card') $TMPL['guideText'] = $this->MultiMenu_guideText;// '본 메뉴 선택시 액션';
        else if($row['resType']=='if') $TMPL['guideText'] =$this->if_MultiMenu_guideText;// '본 조건 해당시 액션';

        if($row['resType']=='card'){
            $TMPL['nav_style'] = 'style="width:auto;"';
        }
    ...
    */
    function getMultiMenuResData($row){
        ...
        while ($R = db_fetch_array($RCD)) {
            ...
            $ttsSpeed_checkBox = new skin('dialog/tts_speed_checkbox');
            ...
            if($row['tmod']){
                ...
                $ttsSelectVal = str_replace('-', 'minus', $R['tts_speed']);

                $TMPL['TTS_item_type'] = 'itemoc';
                $TMPL['TTS_item_uid'] = $R['uid'];
                $TMPL['TTS_value'] = $R['tts_speed'];
                $TMPL['TTS_select_'.$ttsSelectVal] = 'selected';
            }
            ...
            if($resType=='text'){
                ...
                if($row['tmod'] && $row['use_ttsSpeed']) $TMPL['text_checkbox_ttsSpeed'] = $ttsSpeed_checkBox->make();
                ...
                $TMPL['resLink_uid'] = $R['uid'];

                if(stripos($R['varchar_val'], 'javascript:') !== false){
                    $TMPL['blank'] = 'checked';
                    $TMPL['inapp'] = '';
                }
                else{
                ...
                $data['res_link'] = str_replace('javascript:', '', $R['varchar_val']);

                $TMPL['link_checkbox_wlabel'] = (new skin('dialog/link_checkBox'))->make();
                ...
                if($row['tmod'] && $row['use_ttsSpeed']) $TMPL['form_checkbox_ttsSpeed'] = $ttsSpeed_checkBox->make();
        ...
        // html 양식 응답 추가
        if(!isset($data['resHForm_uid']) && $row['resType'] !=='card'){
        ...
        // api header & body 추가
        if(!isset($data['resApi_uid']) && $row['resType'] !=='card'){
    ...
    function getDialogFormResHtml($R){
        ...
        $TMPL['form_ques'] ='';
        $TMPL['form_rec_mark'] = '@';
        $TMPL['formRecTarget_@_selected'] = 'selected';
        $TMPL['formRecTarget_&_selected'] = '';
        $TMPL['formRecTarget_%_selected'] = '';
        $TMPL['form_rec_readonly'] = '';
        $TMPL['form_contextName'] = '';
        $TMPL['form_contextValue'] = '';

        $_rec = $_readonly = $_view = '';

        if($R['text_val'] && $R['text_val'] != "|||"){
            $_arr = explode('|',$R['text_val']);
            $_recMark = substr($_arr[1], 0, 1);
            $_rec = substr($_arr[1], 1);
            $_readonly = $_recMark == '%' ? 'readonly' : '';
            $_view = $_recMark == '%' ? 'hide' : '';
            $_placeholder = $_recMark == '%' ? '인식대상(사용자 발화)' : '인식대상(키워드명)';

            $TMPL['form_ques'] = $_arr[0];
            $TMPL['form_rec_mark'] = $_recMark;
            $TMPL['formRecTarget_@_selected'] = $_recMark == '@' ? 'selected' : '';
            $TMPL['formRecTarget_&_selected'] = $_recMark == '&' ? 'selected' : '';
            $TMPL['formRecTarget_%_selected'] = $_recMark == '%' ? 'selected' : '';
            $TMPL['form_rec_readonly'] = $_readonly;
            $TMPL['form_value_view'] = $_view;
            ...
            if($_recMark == '&') {
                ...
                $_data['bot'] = $R['bot'];
                $_data['html'] = true;
                $_data['apiMethodName'] = $_rec;
                $TMPL['form_rec_input'] = $this->getAPIContextMethods($_data);
            } else {
                $TMPL['form_rec_input'] = '<input name="multiMenuFormRec" class="context-input" placeholder="'.$_placeholder.'" value="'.$_rec.'" '.$_readonly.' data-role="ele-nodeFormRec"/>';
            }
        }else{
            $TMPL['form_rec_input'] = '<input name="multiMenuFormRec" class="context-input" placeholder="인식대상(키워드명)" value="" data-role="ele-nodeFormRec"/>';
        }
        $TMPL['callContext'] = $this->callContext;
    ...
    */
    ...
    function getCheckPrevBotQ($data){
        ...
        // 240320 aramjo r_rec가 %user_input(사용자발화) 또는 apicontext일 경우
        $_mark = substr(trim($r_rec),0,1);
        $_name = substr(trim($r_rec),1);
        if($_mark == "%" || $_mark == "&") {
            // %user_input일 경우 사용자 발화를 컨텍스트에 저장
            if($_mark == "%" && $_name == "user_input") {
                $result['find_go'] = true;
                $this->context[$r_ctxName] = $data['clean_input'];
            }

            // apicontext일 경우
            /*if($_mark == "&" && $_name) {
                $_apiContextVal = $_name.'^'.$r_ctxName.'|'.$r_ctxVal; // 메서드명^컨텍스트명|매칭시킬 응답 apiKey
                $data['apicontext_text_val'] = $_apiContextVal;
                $_apiresult = $this->updateContextArrayFromApi($data);
                if($_apiresult) $result['find_go'] = true;
                else $result['find_stop'] = true;
            }*/
        }
        ...
        $base_wh = $this->getDialogBaseQry($data);
        $R = getDbData($table[$this->module.'dialogNode'], $base_wh." and id='".$q_node."'", 'name, timeout, timeout_msg');
        $result['nodeName'] = $R['name'];
        $result['timeout'] = $R['timeout'];
        $result['timeout_msg'] = $R['timeout_msg'];
        return $result;
    }
    ...
    function getFaqResponse($data) {
        ...
        if($faq_rows > 0 && $data['clean_input']) {
            ...
            $_match = "match(question) against('".$input."*' in boolean mode)";

            $query ="Select uid, question, answer, ".$_match." as score From ".$table[$this->module.'faq']." ";
            $query .="Where vendor=".$data['vendor']." and bot=".$data['bot']." ";
            $query .="and ".$_match." ";
            $query .="Order by score DESC Limit ".(isset($data['all_data']) && $data['all_data'] ? 10 : 1);

            if(isset($data['all_data']) && $data['all_data']) {
                $RCD = db_query($query, $DB_CONNECT);
                while ($R = db_fetch_assoc($RCD)) {
                    $answer[] = array('question'=>$R['question'], 'answer'=>$R['answer']);
                }
            } else {
                $R = db_fetch_assoc(db_query($query, $DB_CONNECT));
                if($R['question'] && $R['answer']) {
                    $bs = getDbData($table[$this->module.'botSettings'], "vendor='".$data['vendor']."' and bot='".$data['bot']."' and name='faqMV'", 'value');
    ...
    function ProcessInput($data) {
        ...
        $exceededCount = !empty($data['exceededCount']) ? $data['exceededCount'] : 0;
            ...
            if(isset($basic_match) && $basic_match){
                ...
                if($this->context["last_botQ"]){
                    ...
                    if($checkBotQ['find_go']|| $checkBotQ['find_stop']){
                        ...
                        // 2024.02.27 spikecow
                        // getCheckPrevBotQ 이게 뭔지는 모르겠으나 혹시나 하여 추가
                        $result['nodeTimeout'] = $checkBotQ['timeout'];
                        $result['nodeTimeoutMsg'] = $checkBotQ['timeout_msg'];
                ...
                if($faq_answer['res']) {
                    ...
                    $result['nodeTimeout'] = '0';
                    $result['nodeTimeoutMsg'] = '';
                ...
                // 2024.02.27 spikecow
                $result['nodeTimeout'] = $dialog_match['timeout'];
                $result['nodeTimeoutMsg'] = $dialog_match['timeout_msg'];
                ...
                if($dialog_match['response'] && $dialog_match['response'][0] && !isset($dialog_match['response'][0][0]['unknown'])){
                    ...
                    if($_SESSION['S_UseChatGPT']) {
                        ...
                        // 20240425 spikecow
                        if($data['msgChatgpt'] != ''){
                            $data['text'] = '<b>'.$data['msgChatgpt'].'</b></br>'.$data['text'];
                        }
                        ...
                        // 2024.03.04 spikecow
                        $exceededCount++;
                        $unrecognizedObject['count'] = $dialog_match['unrecognized_count'];
                        $unrecognizedObject['msg'] = $dialog_match['unrecognized_msg'];
                        $unrecognizedObject['exceededCount'] = $exceededCount;
                        $unrecognizedObject['exceededMsg'] = $dialog_match['exceeded_msg'];
                        ...
                        if($dialog_match['response']){
                            ...
                            $UN = $this->printUnKnownMsg($data, $unrecognizedObject); // Unknown 답변
                            if($data['api']){
                                $result['user_input'] = $clean_input;
                        ...
                        $result['exceededCount'] = $exceededCount;
                    }
        ...
        // aramjo callbot
        } else if('hangup' === $msg_type || 'noinput' === $msg_type || 'sttfail' === $msg_type || 'nomatch' === $msg_type) {
            // 사용자 인풋 log 저장
            $userChat['printType'] ='T';
            $userChat['content'] = $clean_input;
            $userLastChat = $this->addChatLog($userChat);

            if('hangup' === $msg_type) exit;
            ...
            $unrecognizedObject = [
                'msg' => $data['unrecognizedMsg'],
                'count' => (int)$data['unrecognizedCnt'],
                'exceededMsg' => $data['exceededMsg'],
                'exceededCount' => (int)$data['exceededCount'],
            ];
    ...
    //대화상자별 미인식,세션 종료 설정 및 디폴트 세션 종료 설정 추출
    function getNodeSettingInfo($botId, $nodeId)
    {
        $query = "
                    SELECT    
                                node.timeout AS nodeTimeout,
                                node.timeout_msg AS nodeTimeoutMsg,
                                node.unrecognized_count AS unrecognizedCount,
                                node.unrecognized_msg AS unrecognizedMsg,
                                node.exceeded_msg AS exceededMsg,
                                node.fail_msg AS failMsg,
                                timeout.value AS defaultTimeout,
                                timeoutMsg.value AS defaultTimeoutMsg
                    FROM        rb_chatbot_dialogNode AS node
                    INNER JOIN  rb_chatbot_botSettings AS timeout
                            ON  timeout.bot = node.bot AND timeout.name = 'timeout'
                    INNER JOIN  rb_chatbot_botSettings AS timeoutMsg
                            ON  timeoutMsg.bot = node.bot AND timeoutMsg.name = 'timeout_msg'
                    WHERE       node.bot = " . $botId . "
                    AND         node.id = " . $nodeId ;

        try {
            return current($this->getAssoc($query));
    ...
    function getNodeRespond($data){
        ...
        if($dialogActive['type'] == 'T' && $dialogActive['active'] == 0) {
            ...
            $this->lastNodeId = $data['node'];
            $this->lastNodeName = $data['nodeName'];
        }
    ...
    function getDialogResItem($data){
        ...
        if($resType=='text'){
            $R = getDbData($tbl,$wh,'content,uid,bargein,ctiaction,tts_speed'); // 텍스트 답변
            ...
            if($this->bottype == 'call' && $data['api']) {
                ...
                $result[]= array('text',$response,'bargein'=>$bargein, 'next_status'=>$next_status, 'ttsSpeed' => $ttsSpeed);
            } else {
                $result[]= array('text',$response);
            }
            ...
            if($resType=='hMenu'){
                if($total_length >90) $slidePerView = '1';//$total_length/37;//'2.5';
            ...
            if($resType == 'card'){
                $skin = new skin('chat/card_mosaic_list');
            }
            else{
    ...
    */
    function getDialogMatch($data){
        ...
        if($msg_type=='say_hello') {
            ...
            $result['timeout'] = $GDN['timeout'];
            $result['timeout_msg'] = $GDN['timeout_msg'];
            ...
            $result['timeout'] = $R['timeout'];
            $result['timeout_msg'] = $R['timeout_msg'];
            $result['unrecognized_count'] = $R['unrecognized_count'];
            $result['unrecognized_msg'] = $R['unrecognized_msg'];
            $result['exceeded_msg'] = $R['exceeded_msg'];
        }else{
    ...
    */
        ...
        if($FC == '@'){
            ...
            if(isset($data['entityData'])){
                foreach ($data['entityData'] as $E) {
                    ...
                    if($type == 'S') {
                        $entityVal = isset($E[6]) ? $E[6] : (isset($E[3]) ? $E[3] : '');
                    } else {
    ...
    function getDialogNode($data){
        ...
        $result['timeout'] = $TDN['timeout'];
        $result['timeout_msg'] = $TDN['timeout_msg'];
        ...
        foreach ($rows as $row) {
            ...
            if($checkCondition['is_match']){
                ...
                if($row['use_topic']>0){
                    ...
                    // 20240227 spikecow
                    $result['timeout'] = $row['timeout'];
                    $result['timeout_msg'] = $row['timeout_msg'];
                    break;
                }
            }
        }
    ...
    function getEvaluateCondition($data){
        ...
        $op_val = str_replace(' ', '', $data['con_op_val']);
        $con_id = $data['con_id'];
        ...
        if($type =='$'){
            ...
            if($E['has_entity']){
                ...
                for($i=0;$i<$entity_qty;$i++){
                    ...
                    $entityVal_name = str_replace(' ', '', $E['entity_val'][$i][3]); // 문장에서 찾은 entity 벨류 대표명
    ...
    */
    ...
    function getSentenceEntity($data){
        ...
        // 차량번호 체크
        $aResultNumber = $oNumber->getCarNumbers($data);
        if($aResultNumber['is_matched']){
            $data['entityVal'] = $aResultNumber['unitStr']; // entity 값
            $data['entityMatch'] = $aResultNumber['matched']; // 감지 단어
            $data['entityValue'] = $aResultNumber['sum']; // 감지 단어로 인해 실제 계산된 값
            $entityVal[] = $this->getSysEntity($data);
            $has_entity = true;
        }
    ...
    // 잘 몰라요 답변
    function printUnKnownMsg($data, $unrecognizedObject){
        $result = array();
        ...
        if($data['c_ques']) {
            ...
            $dontKnowMsg = '죄송합니다. 다시 한번 말씀해주세요.';
            if ('noinput' !== $data['msg_type']){
                if (!empty($unrecognizedObject['msg']) &&
                    (!empty($unrecognizedObject['count']) && $unrecognizedObject['count'] > 0)
                ){
                    $dontKnowMsg = ($unrecognizedObject['exceededCount'] > $unrecognizedObject['count']) ?
                        $unrecognizedObject['exceededMsg'] : $unrecognizedObject['msg'];
                }
                $dontKnowMsg = !empty($data['failMsg']) ? $data['failMsg'] : $dontKnowMsg;
            }
        }
    ...
    function getDialogMultiItemRow($data){
        ...
        $TMPL['res_frontImg'] = $data['img_url'];
        $TMPL['res_link1'] = $link;
        ...
        if($resType=='card' || $resType=='hMenu'){
            ...
            if($r['varchar_val']){
                ...
                $TMPL['res_cLink']=stripos($cLink, 'javascript:') === false ? 'onclick="chatbot.showFloatLayer(\''.$cLink.'\');"' : 'onclick="window.open(\''.str_replace('javascript:', '', $cLink).'\');"';
            } else {
                // 문진표 자동 링크
                /*if(preg_match('/문진표/', $data['title'])) {
                    $r = getDbData($table[$this->module.'botSettings'], "vendor=".$vendor." and bot=".$bot." and name='use_mediExam'", 'value');
        ...
        elseif($resType == 'card'){
            $skin = new skin('chat/card_mosaic_row');
        }
    ...
    function getMenuRespondRow($data){
        ...
        if($resType=='text'){
            ...
            //callbot--
            $apiResult[] = array('type'=>$resType,'content'=>$parseText,'bargein'=>$bargein, 'next_status'=>$next_status, 'ttsSpeed'=>$ttsSpeed);
        }else if($resType =='form'){
            $_arr = explode('|',$r['text_val']);
            $ques = trim($_arr[0]);
            $rec = $_arr[1];
            ...
            //callbot--
            $apiResult[] = array('type'=>$_result['res_type'], 'content'=>$_result['content'], 'bargein'=>$_result['bargein'], 'next_status'=>$_result['next_status'], 'r_data'=>$_result['r_data'], 'ttsSpeed'=>$ttsSpeed);
        }
    ...
    */
        ...
        if($data['parse']){ // 파싱하는 text
            ...
            if($matches[0]){
                ...
                foreach ($bracket_arr as $key=>$bracket) {
                    $data['b_text'] = trim($bracket);
                    $bracket_parse = $this->getParseText($data);
                    if($bracket_parse != "") {
                        $text = str_replace($bracket,$bracket_parse,$text);
                    } else {
    ...
    function getFavorateQuestionData($data){
        ...
        if($data['d_start'] && $data['d_end']) {
            ...
            if($mod == 'node') {
                ...
                $_wh .=" and (date between '".$d_start."' and '".$d_end."') ";
            }
        }
        ...
        if($mod == 'node') {
            ...
            $limit_query = sprintf("SELECT sentence as content, sum(hit) as hit FROM %s WHERE %s Group by ".$_group." ORDER BY hit DESC, uid ASC LIMIT %s", $tbl,$_wh,$limit);
        } else if($mod == 'word') {
            $limit_query = sprintf("SELECT keyword as content, sum(hit) as hit FROM %s WHERE %s Group by ".$_group." ORDER BY hit DESC, uid ASC LIMIT %s", $tbl,$_wh,$limit);
        }
        $rows = $this->getAssoc($limit_query);
        $list ='';
        if($total){
        ...
        $pageBtn='';
        $pageBtn.='<div class="btn-group" style="padding:0px;height:30px;">';
    ...
    function getAdmBot($data){
        ...
        while($BS = db_fetch_array($BSCD)){
            ...
            else if($BS['name'] =='use_disability') $R['use_disability'] = $BS['value'];

            // 20240214 spikecow
            else if($BS['name'] =='timeout') $R['timeout'] = $BS['value'];
            else if($BS['name'] =='timeout_msg') $R['timeout_msg'] = $BS['value'];

            // 20240425 spikecow
            else if($BS['name'] =='msg_chatgpt') $R['msg_chatgpt'] = $BS['value'];
            // 20240426 spikecow
            //else if($BS['name'] =='use_quick') $R['use_quick'] = $BS['value'];
            else if($BS['name'] =='menu_quick') $R['menu_quick'] = $BS['value'];
        }
        ...
        $R['bot_avatar_bg'] = 'style="background-image: url('.$R['bot_avatar_src'].');background-color:#fff;width:'.$w.';height:'.$h. ';position:relative;' .'"';
        ...
        $_resetBotAvatarImgHtml = '<span class="botAvatar-wrapper btn_reset_botAvatar" data-role="reset_botAvatar"';
        if (strpos($R['bot_avatar_bg'], $_defaultBotAvatarImg) === false) {
            $_resetBotAvatarImgHtml .= '></span>';
        } else {
            $_resetBotAvatarImgHtml .= 'style="display:none;"></span>';
        }

        $R['bot_avatar_img'] = $input . '<span class="botAvatar-wrapper" ' . $role . ' ' . $R['bot_avatar_bg'] . '>' . $_resetBotAvatarImgHtml . '</span>';
    ...
    function getBotDataFromId($bot_id){
        ...
        while($BS = db_fetch_array($BSCD)){
            ...
            else if($BS['name'] =='use_disability') $B['use_disability'] = $BS['value'];

            // 20240214 spikecow
            else if($BS['name'] =='timeout') $B['timeout'] = $BS['value'];
            else if($BS['name'] =='timeout_msg') $B['timeout_msg'] = $BS['value'];

            // 20240425 spikecow
            else if($BS['name'] =='msg_chatgpt') $B['msg_chatgpt'] = $BS['value'];

            // 20240426 spikecow
            //else if($BS['name'] =='use_quick') $B['use_quick'] = $BS['value'];
            else if($BS['name'] =='menu_quick') $B['menu_quick'] = $BS['value'];
        }
        ...
        $result = array(
            ...
            'use_disability'=>$B['use_disability'],
            // 20240214 spikecow
            ...
            // 20240426 spikecow
            //'use_quick'=>$B['use_quick'],
            'menu_quick'=>$B['menu_quick'],
        );
        return $result;
    }
    ...
    */
    function addChatLog($data){
        ...
        if(!$data['chat_uid']) {
            ...
            // 텍스트 형태일 경우
            if($printType=='T'){
                $_today = $date['today'] ? $date['today'] : date('Ymd');
                $chat = $data['last_chat'];
        ...
        if(is_array($data['content'])){
            ...
            foreach ($resArr as $res) {
                if(isset($res['res_type']) && isset($res['content'])) {
                    if($res['res_type'] == 'node' || $res['res_type'] == 'api' || $res['res_type'] == 'context') continue;
                    $type = $res['res_type'];
                    ...
                    if(is_array($res[1])){
                        foreach ($res[1] as $item) {
                            if(is_array($item[0])) {
                                $type = $item[0]['type'];
                                if($type == 'node' || $type == 'api' || $type == 'context') continue;
    ...
    */
    function addBotChatLog($data){
        ...
        if($data['unknown']){
            ...
            $tbl = $table[$m.'unknown'];
            // unknown 문장 공백 제거 검색
            $is_sentence_q = "vendor='".$vendor."' and bot='".$bot."' and replace(sentence, ' ', '')='".preg_replace('/\s+/', '', $sentence)."'";
            //if($roomToken) $is_sentence_q.=" and roomToken='".$roomToken."'"; // roomToken 구분할 경우
    ...
    function getHtmlFormList($row,$mod=''){
        ...
        $aHtml['disability']['disability_request'] = "장애접수 신청";
        $aHtml['disability']['disability_search'] = "장애접수 조회";
        $aHtml['disability']['disability_modify'] = "장애접수 변경";
        $aHtml['disability']['disability_cancel'] = "장애접수 취소";
    ...
    function getHTMLFormRespond($data) {
        ...
        if($data['botData']['bottype'] == 'call' && $data['api']) {
            ...
            $aVarchar_val = explode("_", $r['varchar_val']);
            $_formAction = array_pop($aVarchar_val);
            if($_formAction == "search" || $_formAction == "modify" || $_formAction == "cancel") {
                $TMPL['action'] = $_formAction;
                $skinFile = $data['formType'].'_search';
            } else {
    ...
    function getSysBargein($data) {
        ...
        $ttsSpeed = $bottype == 'call' ? true : false;

        $result = array('bargein'=>$bargein, 'ctiaction'=>$ctiaction, 'ttsSpeed'=>$ttsSpeed);
        return $result;
    }
