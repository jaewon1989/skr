...
$RCD = getDbArray($table[$module.'reply'],$_WHERE,'*',$sort,$orderby,$recnum,$p);
$NUM = getDbRows($table[$module.'reply'],$_WHERE);
$TPG = getTotalPage($NUM,$recnum);

// 언어 설정 데이타  
$languageArr=$chatbot->getLangData();
...
<form name="xlsUploadForm" id="xlsUploadForm" action="/" method="post" target="_action_frame_<?php echo $module?>" enctype="multipart/form-data">	
    ...
    <input type="hidden" name="a" value="_admin/regis_reply_by_excel">
    <span style="display:none;"><input type="file" name="xlsfile" id="input-regis-r" /></span>
</form>
<form name="dumpDataForm" id="dumpDataForm" action="/" method="post" target="_action_frame_<?php echo $module?>" enctype="multipart/form-data">	
    ...
    <input type="hidden" name="act" value="download-replyXlsForm">
</form>
...
<form name="procForm" action="<?php echo $g['s']?>/" method="get" class="form-horizontal rb-form">
	 ...
	 <div class="rb-heading well well-sm search-area" style="position:relative;">
					...
					<div class="col-sm-2">
						...
						<div class="col-sm-4">
	 	 	  	        <button type="button" id="btn-regis" class="btn btn-default btn-sm">
	 	 	  	        	<i class="fa fa-file-excel-o" aria-hidden="true"></i> xls 업로드
	 	 	  	        </button>

	 	 	  	        <a href="#" id="btn-dumpData">[양식 다운로드]</a>
					</div>
	   ...
	   <div class="form-group">
	    ...
	    <a href="#"  data-toggle="modal" data-target="#modal_window" onmousedown="AdvUidSet('<?php echo $R['uid']?>','addReply');" class="pull-right btn-link rb-modal-blog" style="position:absolute;top: 15px;right: 15px"><i class="fa fa-plus"></i> 신규 등록</a>
	 	...
	 	<table class="table table-hover" id="adList-table" style="border-bottom:solid 1px #ddd;">
			...
			<tbody>
				...
				<tr>	<!-- 라인이 체크된 경우 warning 처리됨  -->
				    ...
				    <td><?php echo $chatbot->getCatName($R['quesCat'])?></td>
					<td><?php echo $VND['name']?$VND['name']:'봇톡스' ?></td> 
	<!-- 				<td><?php echo $R['type']=='A'?'단답형':'선택형'?></td>
					<td><?php echo $languageArr[$R['lang']]?></td>
	 -->				<td style="width:40%"><?php echo $R['content']?></td>
				    <td>
                      <a href="#"  data-toggle="modal" data-target="#modal_window" onmousedown="AdvUidSet('<?php echo $R['uid']?>','addReply');" class="btn btn-primary btn-sm rb-modal-blog">수정</a>
				    </td>					
   ...
   <!--목록에 체크된 항목이 없을 경우  fieldset이 disabled 됨-->
	<div class="panel-footer btn-toolbar">
		<fieldset id="list-bottom-fset" disabled> <!--목록에 체크된 항목이 없을 경우  fieldset이 disabled 됨-->
			<div class="btn-group">
				<span  class="btn btn-default adm-act" data-act="delete" data-val=""><i class="fa fa-trash"></i> 삭제</span>
			</div>
		</fieldset>
	    <div class="btn-group pull-right">
	...
	<div class="well text-center text-muted" style="margin-top:15px">
		<i class="fa fa-exclamation-circle fa-lg"></i> 조건에 해당하는 답변가 없습니다.
	</div>
	<?php endif?>
</div>  <!-- // .panel-->
...
// xls 등록 
$(document).on('change','#input-regis-r',function(){
	...
	{
		alert('xls 확장자의 엑셀 파일만 등록할 수 있습니다.    ');
		return false;
	}else{
	   f.submit();	
	}  
...
// 답변 이름,닉네임 클릭시 uid & mod( 탭 정보 : info, main, post 등) 지정하는 함수  
var _mbrModalUid;
var _mbrModalMod;
...
// 답변정보 modal 호출하는 함수 : 위에서 지정한 답변 uid & mod 로 호출한다 .
