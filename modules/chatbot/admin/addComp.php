...
if($uid)
{
	$R = getUidData($table[$module.'company'],$uid);
	$d_start = $R['d_start'];
	$d_end = $R['d_end'];
	...
	// 이미지 처리 
	$photos=$Ad->getUploadPhotoData($module.'_comp',$R['uid']); 
    $IMG=$photos[0]; 
    $photo_uid = $IMG['uid'];
...
<div class="row">
   ...
   <div id="tab-content-view" class="col-sm-8 col-lg-9">
		...
		<input type="hidden" name="a" value="company_regis" />
		...
		<input type="hidden" name="del_photos[]" value="" /> 
	   	 ...
	   	 <?php include $g['path_module'].$module.'/admin/_add_company.php';?>
