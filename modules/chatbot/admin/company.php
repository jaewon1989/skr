...
$RCD = getDbArray($table[$module.'company'],$_WHERE,'*',$sort,$orderby,$recnum,$p);
$NUM = getDbRows($table[$module.'company'],$_WHERE);
...
<form name="procForm" action="<?php echo $g['s']?>/" method="get" class="form-horizontal rb-form">
	 ...
	 <div class="rb-heading well well-sm search-area" style="position:relative;">
		   ...
		   <div class="form-group">
		    ...
		    <a href="#"  data-toggle="modal" data-target="#modal_window" onmousedown="AdvUidSet('<?php echo $R['uid']?>','addComp');" class="pull-right btn-link rb-modal-blog" style="position:absolute;top: 15px;right: 15px"><i class="fa fa-plus"></i> 신규 등록</a>
	 	...
	 	<table class="table table-hover" id="adList-table" style="border-bottom:solid 1px #ddd;">
			...
			<tbody>
				...
				<?php
	              ...
	              $f_img_src = $Ad->getAdFeaturedImgSrc($module.'_comp',$R['uid']);
	              // category 정보
				...
				<tr>	<!-- 라인이 체크된 경우 warning 처리됨  -->
					...
					<td><?php echo $R['name'].'('.$R['id'].')'?></td><!-- main -->	
					<td><?php echo $R['tel']?></td>
					<td><?php echo $R['tel2']?></td>
					<td><?php echo $R['email']?></td>
			  	    <td><?php echo getDateFormat($R['d_regis'],'Y.m.d')?></td>	
				    <td>
                      <a href="#"  data-toggle="modal" data-target="#modal_window" onmousedown="AdvUidSet('<?php echo $R['uid']?>','addComp');" class="btn btn-primary btn-sm rb-modal-blog">수정</a>
				    </td>					
