...
$mdAddinfo=$converter->convert(getContents($R['addinfo'],'HTML')); // blog_data 테이블에는 html 값이 없어서 'HTML' 로 넣어준다.
...
<div class="page-header">
	 <h4>상품<?php echo $R['uid']?'수정':'등록'?></h4>
</div>
<div class="row">
	<form name="procForm" action="<?php echo $g['s']?>/" method="post" target="_action_frame_<?php echo $m?>" enctype="multipart/form-data" onsubmit="return saveCheck(this);" class="rb-form">
	...
	<input type="hidden" name="joint" value="<?php echo $R['joint']?>" />
	      ...
	      <div id="tab-content-view" class="col-sm-8 col-lg-9">
			...
			<li<?php if($_COOKIE['productContent']=='content'||!$_COOKIE['productContent']):?> class="active"<?php endif?>><a href="#content-settings" role="tab" data-toggle="tab" onclick="_cookieSetting('productContent','content');">기본 내용</a></li>
			<li<?php if($_COOKIE['productContent']=='addinfo'):?> class="active"<?php endif?>><a href="#addinfo-settings" role="tab" data-toggle="tab" onclick="_cookieSetting('productContent','addinfo');">추가내용</a></li>
			<li<?php if($_COOKIE['productContent']=='joint'):?> class="active"<?php endif?>><a href="#joint-settings" role="tab" data-toggle="tab" onclick="_cookieSetting('productContent','joint');">관련상품</a></li>
		</ul>
			     <div class="tab-content" id="textarea-wrapper">
			     	      <div class="tab-pane<?php if($_COOKIE['productContent']=='content'||!$_COOKIE['productContent']):?> active<?php endif?>" id="content-settings">
				             <textarea class="form-control" rows="1" id="content" name="content"><?php echo $mdContent?></textarea>
	                       </div>
	                       <div class="tab-pane<?php if($_COOKIE['productContent']=='addinfo'):?> active<?php endif?>" id="addinfo-settings">
				             <textarea class="form-control" rows="1" id="addinfo" name="addinfo"><?php echo $mdAddinfo?></textarea>
	                       </div>
	                       <div class="tab-pane<?php if($_COOKIE['productContent']=='joint'):?> active<?php endif?>" id="joint-settings">
	                             <?php include $g['path_module'].$module.'/admin/_regis_related_product.php';?>
	                       </div>
	                 </div>      
		      </div>
...
function saveCheck(f)
{
	...
	if (f.category.value == '')
	{
		alert('등록할 매장 분류(카테고리)를 선택해 주세요.       ');
		f.category.focus();
		return false;
	}
	...
	// 첨부파일 uid 를 upfiles 값에 추가하기  
      ...
      if(attachfiles){
          var new_upload=upload+attachfiles;
           new_upload=new_upload.replace(/,/gi,'');// 콤마 제거 
          $('input[name="upload"]').val(new_upload);
      }
