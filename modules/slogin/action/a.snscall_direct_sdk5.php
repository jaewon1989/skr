...
if ($d['social']['use_f'])
{
	...
	if ($type == 'f')
	{ 
		  $app_id =$d['social']['key_f'];
        $app_secret = $d['social']['secret_f'];
        $redirect_uri = urlencode($g['url_root'].'/?r='.$r.'&m='.$m.'&a=snscall_direct&facebook=Y');
        
        // Autoload the required files
         require_once ( '/modules/social/oauth/facebook-sdk-php-v4/facebook-php-sdk-v4-4.0-dev/autoload.php' );
	...
	{
	   // Autoload the required files
      require_once $g['path_module'].'social/oauth/facebook-sdk-php-v4/facebook-php-sdk-v4-4.0-dev/autoload.php';
	   ...
	   $f_mbrid = 'fb_'.$d['social']['key_f'].'_user_id';
		...
		if ( isset( $session ))
		{
         ...
         // 회원정보 가져오기 
			$request = ( new FacebookRequest( $session, 'GET', '/me?fields=link,id,name,email,gender,picture' ) )->execute();
         // Get response as an array
         $FR = $request->getGraphObject()->asArray();
			//$FR=$FC->api('/me?fields=link,id,name,email,gender,picture');
			$id =$FR['id'];
			$name =$FR['name'];
			$email =$FR['email'];
			...
			$gender=$FR['gender'];
			$profile_url=$FR['link'];
         $_SESSION[$f_token]=$_SESSION['fb_token'];
         $_SESSION[$f_mbrid]=$id;
			...
			if ($my['uid'])
			{
				...
				if($_ISSNS['memberuid'])
				{
	               ...
	               include_once $g['path_core'].'function/rss.func.php';
					...
					if ($picdata) $photo=setAvatar($picdata,$id,'small');
...
if ($d['social']['use_g'])
{
	...
	if ($google == 'Y')
	{
       ...
       $redirect_uri = $g['url_root'].'/?r='.$r.'&m='.$m.'&a=snscall_direct&google=Y';
		if ($denied)
		{
