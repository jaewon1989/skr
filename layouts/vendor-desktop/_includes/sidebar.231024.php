...
if($_member) {
    ...
    while($_M1 = db_fetch_array($M1)){
        ...
        $_tempMenu = ['id'=>$_M1['id'], 'name'=>$_M1['name'], 'aSideMenu'=>[]];
        ...
        while($_M2=db_fetch_array($M2)){
            ...
            $_M2['name'] = $bottype == 'call' ? str_replace('챗봇', '콜봇', $_M2['name']) : $_M2['name'];
            $_tempMenu['aSideMenu'][] = ['id'=>$_M2['id'], 'name'=>$_M2['name'];
        }
        $_botMenu[] = $_tempMenu;
    }
...
<div class="navbar-default sidebar" role="navigation">
    <div class="sidebar-nav navbar-collapse slimscrollsidebar">
        ...
        <?} else {?>
            ...
            <?php
                $g['side_menu'][] = $_M1['id'];
                if($_M1['id'] =='main') $menu_url = $g['s'].'/adm/'.$_M1['id'].'?bottype='.$bottype;
                else $menu_url = $g['s'].'/adm/'.$_M1['id'];

                if(($_M1['id'] =='dashboard' || $_M1['id'] =='analysis' || $_M1['id'] =='monitering') && $roleType =='topic') continue;

                if(in_array($_M1['id'], $mymenu)) {
            ?>
            <li class="sideMenu">
                <a href="<?php echo $menu_url?>" class="waves-effect<?php if($_M1['is_child']):?> has-arrow<?php endif?>" <?php if($_M1['is_child']):?> data-child="true"<?php endif?>>
                    <i class="fa <?php echo $menu_icon[$_M1['id']]?> fa-fw" aria-hidden="true"></i>
                    <span class="hide-menu"><?php echo $chatbot->getRoleTypeName($_M1['name'])?></span>
                </a>
                <?php if($_M1['is_child']){?>
                <?php
                    if($roleType=='topic') $_wh2 = "parent='".$_M1['uid']."' and hidden=0 and id in ('config','graph','intentSet','entitySet')";
                    else $_wh2 = "parent='".$_M1['uid']."' and hidden=0";

                    $M2=getDbArray($table['s_menu'],$_wh2,'uid,name,id,is_child','gid','asc','',1);
                    $aSideMenu = array();
                    while($_M2=db_fetch_array($M2)){
                        $aSideMenu[] = $_M2;
                    }
                ...
                <ul class="submenu">
                    <?php
                        foreach($aSideMenu as $_M2){
                            if($bottype == 'call' && ($_M2['id'] == 'skin' || $_M2['id'] == 'intro')) continue;
                            $g['side_menu'][] = $_M2['id'];
                            $_M2['name'] = $bottype == 'call' ? str_replace('챗봇', '콜봇', $_M2['name']) : $_M2['name'];
                            if(in_array($_M2['id'], $mymenu)) {
                    ?>
                    <li <?php if($_page ==$_M2['id']){?>class="active"<?}?>>
                        <?php if($_M2['id']=='intent'||$_M2['id']=='entity'||$_M2['id']=='context'||$_M2['id']=='node'){?>
                         <a href="##" class="waves-effect">
                            <i class="fa <?php echo $menu_icon[$_M2['id']]?> fa-fw" aria-hidden="true"></i>
                              <span class="hide-menu"><?php echo $_M2['name']?></span>
                         </a>
                        <?}else{?>
                         <?php
                            if($_M2['id'] =='config') $menu2_url = $g['s'].'/adm/'.$_M2['id']; //'http://'.$_SERVER['HTTP_HOST'].'/adm/'.$_M2['id'];
                            else $menu2_url = $g['s'].'/adm/'.$_M2['id'];
                        ?>
                        <a href="<?php echo $menu2_url?>" class="waves-effect">
                            <i class="fa <?php echo $menu_icon[$_M2['id']]?> fa-fw" aria-hidden="true"></i>
                              <span class="hide-menu"><?php echo $_M2['name']?></span>
                         </a>
                        <?}?>
