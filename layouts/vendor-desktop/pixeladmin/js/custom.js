    ...
    $(function() {
            ...
            var e = window.location,
                i = $("ul.nav a").filter(function() {
                    return this.href == e.href // || 0 == e.href.indexOf(this.href)
                }).addClass("active").parent().parent().addClass("in").parent().addClass("active")
                ii = "";
			$("ul.nav > li").each(function(){
				if(!!$(this).attr('class') && $(this).hasClass('active') && !!$(this).find('a img') && !!$(this).find('a img').attr('src')){
					$(this).find('a img').attr('src', $(this).find('a img').attr('src').replace("-r.svg", ".svg").replace(".svg", "-r.svg"));
				}
			});

            if(i.find(".active img").length > 0) {
                i.is("li") && i.addClass("active");
                ii = (i.find('li.active a img').length == 1)? "-r" : "-w";
                i.find(".active img").attr("src", i.find(".active img").attr("src").replace(ii+".svg", ".svg").replace(".svg", ii+".svg"));

                $("ul.nav").on('mouseover', "li.sideMenu", function(e){
                    if(!$(this).find("a").hasClass('active')){
                        let _src = $(this).find("img").attr("src").replace("-r.svg", ".svg").replace(".svg", "-r.svg");
                        $(this).find("img").attr("src", _src);
                    }
                }).on('mouseout', "li.sideMenu", function(e){
                    if(!$(this).find("a").hasClass('active')){
                        let _src = $(this).find("img").attr("src").replace("-r.svg", ".svg");
                        $(this).find("img").attr("src", _src);
                    }
                });
            }
