...
function openRegisModal(MOS,addData){
    $('#modal-regis').modal();
    ...
    if(MOS=='ios'||MOS=='android'){
           ...
           $('#modal-regis').find('[data-role="preview-ul"]').append(addData); // 업로드 리스트 추가  
    } 
...
function Success_App_Upload(arg){
    ...
    if(code==0){
        ...
        $.each(files,function(key,list){
            ...
            var type = list_arr[2];
            if(type==2){
                $('input[name="regisType"]').val('photo');
                ...
                var video_frame = '';
                video_frame += '<video width="300" height="200" controls>';
                video_frame += '<source src="'+src+'" type="video/mp4">';
                video_frame += '</video>';
                var video_info = 'video^^비디오^^shot^^'+video_frame;
                $('input[name="regisType"]').val('video');
                $('input[name="links"]').val(video_info);
                file_list += '<li>';
                file_list += '<input type="hidden" name="videos[]" value="'+uid+'" data-type="'+type+'"/>';
                file_list += video_frame;
                file_list += '</li>';
            }
...
$(document).on('tap','[data-toggle="goCamera"]',function(){
    ...
    else openRegisModal(MOS,addData); 
});
