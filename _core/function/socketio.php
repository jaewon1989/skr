...
class SocketIO
{
    ...
    public function __construct($host = null, $port = null, $path = "/socket.io/EIO=3") {
        if(strpos($host, SocketIO::SSL_PROTOCOLE) !== false) {
            $this->protocole = SocketIO::SSL_PROTOCOLE;
            $this->host = str_replace(SocketIO::SSL_PROTOCOLE, '', $host);
        } else if(strpos($host, SocketIO::TLS_PROTOCOLE) !== false) {
            $this->protocole = SocketIO::TLS_PROTOCOLE;
            $this->host = str_replace(SocketIO::TLS_PROTOCOLE, '', $host);
        } else {
            $this->protocole = SocketIO::NO_SECURE_PROTOCOLE;
            $this->host = $host;
        }        
        $this->port = $port;
        $this->path = $path;
    ...
    {
        if(!in_array($protocole, array(SocketIO::NO_SECURE_PROTOCOLE, SocketIO::SSL_PROTOCOLE, SocketIO::TLS_PROTOCOLE)))
        {
    ...
    private function send()
    {
        ...
        $fd = stream_socket_client("{$this->protocole}{$this->host}" . ':' . $this->port, $errno, $errstr, 10, STREAM_CLIENT_CONNECT, $context);        
