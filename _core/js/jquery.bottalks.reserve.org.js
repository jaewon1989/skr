...
var reservedInfo = {};
var apiData = {};
var hreserve_action_url = rooturl+"/?r="+raccount+"&m="+module+"&a=get_reserve";
var msgBox = chatbot.chatLogContainer;
var prevForm;
var lastChatForm;
var useDates = {};
var dateRange = false;
...
$(document).on("click", ".btn_rateplan", function() {
    if($(this).closest(".rateplan").hasClass("on")) {
        ...
        $(this).parents("fieldset").find(".rateplan").removeClass("on");
...
function getPricePerson(obj) {
    ...
    $(obj).find("input.extra_adult_price").val($_add_adult_price);
    $(obj).find("input.extra_child_price").val($_add_child_price);
    $(obj).find("input.total_price").val($nTotalPrice).parent(".total_price").find("em").text(commaSplit($nTotalPrice));
}
...
$(document).on("click", ".submit_reserve", function() {
    $this = $(this);
    ...
    apiData['vendor'] = chatbot.vendor;
    apiData['botid'] = chatbot.botId;
    apiData['bot'] = chatbot.botUid;
    apiData['dialog'] = chatbot.dialog;
    apiData['roomToken'] = chatbot.roomToken;
    apiData['cmod'] = chatbot.cmod;
    apiData['bot_skin'] = chatbot.bot_skin;
...
function getUserMsg() {
    var load_msg = chatbot.getLoadMsgTpl();
    $(msgBox).append(load_msg);
    var _dd = {input_type: 'hform'};
    chatbot.resetScrollTop(_dd);
}
...
function getFormDisabled(obj) {
    ...
    $(obj).find("input, button, div.btn_radio, .btn_cnt").prop("disabled", true).addClass("nopointer");
...
function getReserveSendAjax(data) {
    ...
    //data['r_data'] = encryption.encrypt(data['r_data'], data['nonceVal']);
    
    return $.post(hreserve_action_url,{
        pData : data
    },function(response) {
...
function showBotHtmlFormMsg(result) {    
    $(msgBox).append($(result.msg)).promise().done(function() {
        ...
        if(!chatbot.cmod && chatbot.options.use_chatting=='on' && chatbot.admSockId){
            setTimeout(function() {
                var msg = $(msgBox).children(".cb-chatting-form:last-child").clone().wrapAll("<div/>").parent().html();
                chatbot.socketSend({"role":"chat_log_send", "roomToken":self.roomToken, "to":chatbot.admSockId, "msg":msg});
            },200);
        }
