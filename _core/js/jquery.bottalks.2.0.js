(function (factory) {
    ...
    var Bottalks = {
        ...
        events: {
            ...
            'click [data-role="cardType-resItem"]' : 'getMenuRespond', // card  item 클릭
        ...
        init: function(options, el) {
            ...
            this.errorMsg = this.options.error_msg === '' ? '잘못된 접근입니다.' : this.options.error_msg;
            this.timeout = this.options.timeout === '' ? 0 : this.options.timeout;
            this.timeoutMsg = this.options.timeout_msg === '' ?  '죄송해요. </br> 말씀이 없으셔서 챗봇이 종료되었어요. </br> 다시 이용을 원하시면 새로고침을 눌러주세요.' : this.options.timeout_msg;
            ...
            this.msgChatgpt = this.options.msg_chatgpt;
        },
        ...
        getBotMsg: function (data) {
            ...
            _data['errorMsg'] = this.errorMsg;
            _data['exceededCount'] = this.exceededCount;
            _data['msgChatgpt'] = this.msgChatgpt;
            ...
            $.post(rooturl+'/?r='+raccount+'&m='+this.module+'&a=get_reply',{
                ...
                const timeoutObject = {
                    timeout: reply.nodeTimeout,
                    timeoutMsg: reply.nodeTimeoutMsg
                };
                self.exceededCount = reply.exceededCount;
                if($.isArray(response)){
                    $.each(response,function(i,resItem){
                        ...
                        if($.isArray(itemCont)){
                            $.each(itemCont,function(i,item){
                                ...
                                self.showBotMsg(resData, timeoutObject);
                            });
                        }else{
        ...
        // Print Bot Msg
        showBotMsg : function(data, timeoutObject){
            var self = this;
            var user_input = this.userInputEle;
            var msgBox = this.chatLogContainer;
            var msg;
            ...
            this.initTimeout("bot", timeoutObject);
        },
        ...
        showUserMsg : function(data){
            ...
            if(inputer == 'user') {
                ...
                this.initTimeout("user", timeoutObject);
        ...
        getMenuRespond: function(e){
            ...
            var msg_data = self.setMsgData(data['title'],'hMenu');
               ...
               pData: data,
                ...
                $.each(content,function(i,res){
                    ...
                    if(resType=='text'||resType=='img'||resType=='tel'||resType=='form'||resType=='hform'){
                        ...
                        self.showBotMsg(_data, timeoutObject);
        ...
        getNodeRespond: function(data){
            ...
            $.post(rooturl+'/?r='+raccount+'&m='+self.module+'&a=get_reply',{
               pData: _data
                ...
                var content = result.content;
                const timeoutObject = {
                    timeout: result.nodeTimeout,
                    timeoutMsg: result.nodeTimeoutMsg
                };
        ...
        showErrorMsg: function (msg) {
            //$(this.chatLogContainer).append(this.getMsgTpl({"role": "bot", "type": "text", "msg": msg}));
            $("#commonLayerPanel p").html(msg);
            $("#bgWrap").addClass("active");
            $("#commonLayerPanel").addClass("active");
        },

        showFloatLayer: function (link) {
            $("#floatingLayerPanel input").val(link);
            $("#floatingLayerPanel iframe").attr('src', link);
            $("#bgWrap").addClass("active");
            $("#floatingLayerPanel").addClass("active");
        },

        hideFloatLayer: function () {
            $("#floatingLayerPanel input").val('');
            $("#floatingLayerPanel iframe").attr('src', '');
            $("#bgWrap").removeClass("active");
            $("#floatingLayerPanel").removeClass("active");
        },

        initTimeout : function(type, obj){
            const self = this;
            let timeout;
            let timeoutMsg;

            if(obj.timeout !== null && obj.timeout !== '' && obj.timeout !== '0' && obj.timeout !== 'undefined'){
                timeout = obj.timeout;
                timeoutMsg = obj.timeoutMsg;
            }else{
                timeout = self.timeout;
                timeoutMsg = self.timeoutMsg;
            }

            if (parseFloat(timeout) === 0 || ["monitoring",'dialog', "callbotChat",'chatbotlog'].indexOf(self.cmod) !== -1 ){
                clearInterval(self.timerObject);
                return;
            }

            if (self.timerObject != null) clearInterval(self.timerObject);
            self.timerObject = setInterval(() => {
                self.showErrorMsg(timeoutMsg);
                clearInterval(self.timerObject);
            }, (parseFloat(timeout) * 1000)); // * 1 * 1000*/
        }
