...
function isValidPhoneNumber(chStr) {
    var arrMatch = chStr.replace(/-/g, "").match(/^((01[016789])|(02|0[3-9]{1}[0-9]{1}))[0-9]{3,4}[0-9]{4}$/g);
    if (arrMatch == null) return false;
    else return true;
}

function getRmSpecialChar(chStr) {
    return chStr.replace(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"\s]/gi, '');
}

function userClearInput(text) {
    text = text.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, "");
    text = text.replace(/\<|\>|\(|\)|\"|\'|\;|#|&|%/g, "");
    // 주민번호,외국인
    text = text.replace(/((?=\D)|^|\b)(\d{2})((0[13578]|1[02])(0[1-9]|[12][0-9]|3[01])|(0[469]|11)(0[1-9]|[12][0-9]|3[0])|(02)(0[1-9]|[12][0-9]))(-|\s)?([1-8])(\d{6})((?=\D)|$|\b)/g, "$2$3$10$11******");
    // 휴대폰
    text = text.replace(/((?=\D)|^|\b)(01)([01689])(-|\s)?(\d{1}|\d{2})(\d{2})(-|\s)?(\d{4})((?=\D)|$|\b)/g, "$2$3$4$5**$7****");
    return $.trim(text);
}
...
function getFormData(obj) {
    ...
    var name;
    var $targets;
    $targets = $(obj).find('input, select, textarea');
    $targets.each(function () {
        ...
        if ($(this).prop('type') == 'checkbox') {
            ...
            if ($(this).is(':checked')) {
                ...
                if ($(this).data('unchecked')) value = $(this).data('unchecked');
            }
            if(typeof object[this.name] == 'undefined') object[this.name] = [];
            if (typeof value != 'undefined') object[this.name].push(value);
            return true;
        }
        
        if ($(this).prop('type') == 'radio') {
            object[this.name] = $('input[name="' + this.name + '"]:checked').val();
            if (object[this.name] == null) delete object[this.name];
            return true;
        }
        ...
        if (!value && $(this).data('set')) value = $(this).data('set');
        if (object[this.name] !== undefined) {
            if (!object[this.name].push) object[this.name] = [object[this.name]];
