...
if(!$g['https_on']) getLink($g['ssl_root'].'/?'.$_SERVER['QUERY_STRING'],'','','');
...
if ($_SESSION['mbr_uid'])
{
    $dd = getUidData($table['s_mbrid'],$_SESSION['mbr_uid']);
...
$g['meta_img'] = $g['s'].'/files/bottalks_logo.png'; //getMetaImage($_SEO['image_src']);
	...
	if ($d['admin']['ssl_type'] == 2)
	{
		if ($_HP['uid'])
		{
			if (strpos(',,'.$d['admin']['ssl_page'].',',','.$_HP['id'].','))
			{
				...
				if($g['https_on']) getLink(str_replace(':'.$d['admin']['ssl_port'],'',str_replace('https://','http://',$g['url_root'])).'/?'.$_SERVER['QUERY_STRING'],'','','');
			}
		}
		else if ($_HM['uid'])
		{
